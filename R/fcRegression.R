...
#' \deqn{g(E(Y_i)) = \sum_{l=1}^{L} \int_{\Omega_l} \beta_l(t) X_{li}(t) dt + \sum_{j=1}^{J} \gamma_j Z_{j.i} + \gamma_0}
...
#' @param Z Scalar covariate(s), can be NULL (when there's no scalar covariate), an atomic vector (when only one scalar covariate), a matrix or data frame, recommended form is a data frame with column name(s).
...
#' @import stats
fcRegression = function(Y, FC, Z, formula.Z = NULL, family = gaussian(link = "identity"), basis.type = c('Fourier','Bspline'), basis.order = 6L, bs_degree = 3){
