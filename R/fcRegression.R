...
#' Model allows one or multiple functional covariate(s) as fixed effect(s), and zero, one, or multiple scalar-valued fixed or random effect(s).
...
#' @param formula.Z A formula without the response variable, contains only scalar covariate(s) (or intercept), use the format of lme4 package if random effects exist. e.g. ~ Z_1 + (1|Z_2). (See \code{\link[lme4]{lmer}} and \code{\link[lme4]{glmer}})
...
#' When using Fourier basis \eqn{\frac{1}{2},\sin k t, \cos k t, k = 1,\dots,p_f}, basis.order is the number \eqn{p_f}.
