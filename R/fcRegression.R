...
#' Model allows one or multiple functional covariate(s) as fixed effect(s), and non-functional (scalar) fixed or random effect(s).
#' @details Solve linear models with functional covariates below
#' \deqn{g(E(Y_i)) = \sum_{l=1}^{L} \int_{\Omega_l} \beta_l(t) X_{li}(t) dt + \sum_{j=1}^{J} \gamma_j Z_{ji} + \gamma_0}
...
#'
#' @param Y Response variable, can be an atomic vector, a one-column matrix or data frame, recommended form is a one-column data frame with column name.
#' @param FC Functional covariate(s), can be a "functional_variable" object or a matrix or a data frame or a list of these object(s).
#' @param Z Scalar covariate(s), can be NULL or not input (when there's no scalar covariate), an atomic vector (when only one scalar covariate), a matrix or data frame, recommended form is a data frame with column name(s).
#' @param formula.Z A formula without the response variable, contains only scalar covariate(s) (or intercept), use the format of lme4 package if random effects exist. e.g. ~ Z_1 + (1|Z_2).
#'     If not assigned, include all scalar covariates and intercept term as fixed effects.
#' @param family A description of the error distribution and link function to be used in the model, see \code{\link[stats]{family}}.
...
#'     \item{FC.BasisCoefficient}{A list of Fourier_series or bspline_series object(s), represents the functional linear coefficient(s) of the functional covariates.}
...
#' @import stats
fcRegression = function(Y, FC, Z, formula.Z , family = gaussian(link = "identity"), basis.type = c('Fourier','Bspline'), basis.order = 6L, bs_degree = 3){
