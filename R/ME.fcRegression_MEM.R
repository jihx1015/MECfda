...
#' and the distribution assumption on the observed measurement is relaxed to encompass the exponential family rather than being limited to the Gaussian distribution.
#' The MEM approach employs point-wise (UP_MEM) and multi-point-wise (MP_MEM) estimation procedures to avoid potential computational complexities caused by analyses of multi-level functional data and computations of potentially intractable and complex integrals.
#' @references Luan, Yuanyuan, et al. "Scalable regression calibration approaches to correcting measurement error in multi-level generalized functional linear regression models with heteroscedastic measurement errors." arXiv preprint arXiv:2305.12624 (2023).
...
#' @param method The method to construct the substitution \eqn{X}. Avaiable options: 'UP_MEM', 'MP_MEM', 'average'.
...
#' @param d The number of time points invovled for MP_MEM (defualt and miniumn is 3).
#' @param family.W Distribution of \eqn{W} given \eqn{X},  Avaiable options: "gaussian","poisson".
#' @param family.Y A description of the error distribution and link function to be used in the model, see \code{\link[stats]{family}}.
#' @param formula.Z A formula without the response variable, contains only scalar covariate(s), use the format of lme4 package if random effects exist. e.g. ~ Z_1 + (1|Z_2). If not assigned, include all scalar covariates and intercept term as fixed effects.
...
#'                           basis.type = 'Bspline')
#'
ME.fcRegression_MEM = function(data.Y, data.W, data.Z, method = c('UP_MEM', 'MP_MEM', 'average'), t_interval = c(0,1), t_points = NULL, d = 3,
                               family.W = c("gaussian","poisson"), family.Y = "gaussian", formula.Z = NULL,
  ...
  data.analysis = cbind(data.Y,data.Z)
