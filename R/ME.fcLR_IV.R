...
#' @references Tekwe, Carmen D., et al. "Instrumental variable approach to estimating the scalar‐on‐function regression model with measurement error with application to energy expenditure assessment in childhood obesity." Statistics in medicine 38.20 (2019): 3764-3781.
...
#' @param CI.bootstrap Whether to return the confidence using bootstrap method. Default is FALSE.
#'
#' @return Return a ME.fcLR_IV class object. It is a list that contains the following elements.
#'    \item{beta_tW}{Estimate.}
...
ME.fcLR_IV = function(data.Y, data.W, data.M,
  ...
  M_ic <- scale(M_i,scale = T)
  W_ic <- scale(W_i,scale = T)
  Y_c <-  scale(Y, scale = T )
  ...
  if(CI.bootstrap){
    ...
    repeat{
      ...
      M.boot_ic <-  scale(M.boot_i,scale = T)
      W.boot_ic <-  scale(W.boot_i,scale = T)
      Y.boot_c <-   scale(Y.boot,scale = T)
