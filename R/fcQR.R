...
#' \deqn{Q_{Y_i}(\tau) = \sum_{l=1}^L\int_\Omega \beta_l(\tau,t)dt + \gamma_j(\tau) Z_{j.i} + \gamma_0(\tau)}
#' where \eqn{\tau\in(0,1)} is the quantile.
#' Model may contains one or more functional covariates.
#'
...
#' @param formula.Z A formula without the response variable, contains only scalar covariate(s). If not assigned, include all scalar covariates and intercept term.
...
#' @import stats
fcQR = function(Y, FC, Z, formula.Z = NULL, tau = 0.5, basis.type = c('Fourier','Bspline'), basis.order = 6L, bs_degree = 3){
