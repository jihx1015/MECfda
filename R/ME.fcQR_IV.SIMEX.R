...
#' @description
#' Perform a two-stage strategy to correct the measurement error of a function-valued covariate and then fit a linear quantile regression model.
#' In the first stage, an instrumental variable is used to estimate the covariance matrix associated with the measurement error.
#' In the second stage, simulation extrapolation (SIMEX) is used to correct for measurement error in the function-valued covariate. \cr
#' See detailed model in the reference.
#' @references Tekwe, Carmen D., et al. "Estimation of sparse functional quantile regression with measurement error: a SIMEX approach." Biostatistics 23.4 (2022): 1218-1241.
#'
...
#' @param data.W A dataframe or matrix, represents \eqn{W}, the measurement of \eqn{X}. Each row represents a subject. Each column represent a measurement (time) point.
#' @param data.Z Scalar covariate(s), can be NULL (when there's no scalar covariate), an atomic vector (when only one scalar covariate), a matrix or data frame, recommended form is a data frame with column name(s).
#' @param data.M A dataframe or matrix, represents \eqn{M}, the instrumental variable. Each row represents a subject. Each column represent a measurement (time) point.
#' @param tau Quantile, default is 0.5.
...
#' @param formula.Z A formula without the response variable, contains only scalar covariate(s), no random effects. If not assigned, include all scalar covariates and intercept term.
...
#' For the detailed form for Fourier and b-splines basis, see \code{\link{fourier_basis_expansion}} and \code{\link{bspline_basis_expansion}}.
#' @param basis.order Indicate number of the function basis.
#' When using Fourier basis \eqn{\frac{1}{2},\sin k t, \cos k t, k = 1,\dots,K}, basis.order is the number \eqn{K}.
#' When using b-splines basis \eqn{\{B_{i,p}(x)\}_{i=-p}^{k}} basis.order is the number of splines, equal to \eqn{k+p+1}. (same as arguement \code{df} in \code{\link[splines]{bs}}.)
...
#' @param bs_degree Degree of the piecewise polynomials if use b-splines basis, default is 3. See \code{degree} in \code{\link[splines]{bs}}.
#'
#' @return Returns a ME.fcQR_IV.SIMEX class object. It is a list that contains the following elements.
#' \item{coef.X}{A Fourier_series or bspline_series object, represents the functional coefficient parameter of the functional covariate.}
...
#' \item{t_interval}{A 2-element vector, represents an interval, means the domain of the functional covariate.}
...
ME.fcQR_IV.SIMEX = function(data.Y, data.W, data.Z, data.M, tau = 0.5,
  ...
  M.new = data.M/matrix(rep(delta_t.hat,n), nrow=n, ncol=t, byrow = T)
  ...
  gamma.simex = lapply(seq(1:B), function(b){
    ret = sapply(lambda, function(s) {
      ...
      model =  try(rq(formula = fmla, data = data.funRegress, tau = tau),T)
  ...
  zlevels = lapply(data.Z.1[,sapply(data.Z.1,is.factor),drop = F],levels)
